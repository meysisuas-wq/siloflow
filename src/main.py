...
async def lifespan(app: FastAPI):
    logger.info("Starting SiloFlow")
    await init_db()
    yield
    await close_db()

def create_app() -> FastAPI:
    app = FastAPI(title="SiloFlow", description="Precision Agriculture Intelligence Platform",
    ...
    async def health():
        return {"status": "healthy", "service": "siloflow", "version": "1.0.0"}
